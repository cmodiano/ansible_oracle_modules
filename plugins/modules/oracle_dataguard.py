...
options:
  ...
  oracle_home:
    ...
    aliases: ['oh']
  dgmgrl_user:
    description: Username for DGMGRL connection
    required: false
  dgmgrl_password:
    description: Password for DGMGRL connection
    required: false
  dgmgrl_connect_identifier:
    description: Connect identifier for DGMGRL (e.g. host:port/service)
    required: false
  dgmgrl_as:
    description: DGMGRL connection privilege
    default: sysdg
...
- name: Get Data Guard configuration status
  oracle_dataguard:
...
- name: Get Data Guard status via SQL
  oracle_dataguard:
    mode_dg: sql
    mode: sysdba
    state: status
